...
class workshopController {
  static async getWorkshopDetails(req, res) {
    try {
      ...
      const result = await getWorkshopDetails(
        workshop_id,
        company_id,
        schedule_id
      );
  ...
  static async getWorkshopsByCompanyIdOrUserId(req, res) {
    try {
      ...
      const result = await getWorkshopsByCompanyIdOrUserId(
        company_id,
        user_id,
        page,
        limit,
  ...
  static async getWorkshopDatesByCompanyIdOrUserId(req, res) {
    try {
      ...
      const result = await getWorkshopDatesByCompanyIdOrUserId(
        company_id,
        user_id
      );
      return res.status(result.code).json(result);
        ...
        try {
          ...
          await ActivityLogService.createLog({
            ...
            description: `Workshop "${
              workshopDetails[0]?.title || id
            }" (ID: ${id}) updated. Fields changed: ${updatedFields}`,
          });
        } catch (logError) {
      ...
      if (result.status) {
        try {
          ...
          await ActivityLogService.createLog({
            ...
            module_name: "workshops",
            action: "create",
            description: `Workshop "${title}" created`,
          });
        } catch (logError) {
          console.error("Error creating activity log:", logError);
        }
    ...
    try {
      ...
      if (!company_id || !date || !time || !workshop_id || !duration_minutes) {
        return res.status(400).json({
          ...
          code: 400,
          message:
            "Company ID, date, time, workshop ID, and duration minutes are required",
          data: null,
        });
      }
      ...
      if (result.status) {
        try {
          ...
          await ActivityLogService.createLog({
            ...
            action: "schedule",
            description: `Workshop "${
              workshopDetails[0]?.title || ""
            }" (ID: ${workshop_id}) scheduled for company "${
              companyDetails[0]?.company_name || ""
            }" (ID: ${company_id}) on ${formattedDateTime}`,
          });
        } catch (logError) {
          console.error("Error creating activity log:", logError);
          // Continue with the response even if logging fails
  ...
  static async cancelWorkshopSchedule(req, res) {
    try {
      ...
      const result = await updateWorkshopScheduleStatus(
        schedule_id,
        "cancelled",
        reason
      );
      ...
      if (result.status && scheduleDetails.length > 0) {
        try {
          ...
          await ActivityLogService.createLog({
            ...
            action: "cancel",
            description: `Workshop "${
              scheduleDetails[0].workshop_title
            }" (ID: ${scheduleDetails[0].workshop_id}) for company "${
              scheduleDetails[0].company_name
            }" (ID: ${
              scheduleDetails[0].company_id
            }) scheduled for ${formattedDateTime} was cancelled${
  ...
  static async rescheduleWorkshop(req, res) {
    try {
      const { schedule_id, new_start_time, new_end_time, duration_minutes } =
        req.body;
      const user = req.user;
      ...
      if (
        ...
        (isNaN(duration_minutes) || duration_minutes <= 0)
      ) {
        return res.status(400).json({
          status: false,
          code: 400,
      ...
      if (result.status && scheduleDetails.length > 0) {
        try {
          ...
          const oldEndTime = new Date(
            scheduleDetails[0].end_time
          ).toLocaleString();
          const newStartTimeFormatted = new Date(
            new_start_time
          ).toLocaleString();
        ...
        try {
          ...
          if (status === "cancelled") {
            ...
            description = `Workshop "${
              ...
              scheduleDetails[0].company_name
            }" (ID: ${scheduleDetails[0].company_id}) status changed from "${
              scheduleDetails[0].current_status
            }" to "${status}"${reason ? `. Reason: ${reason}` : ""}`;
          }
