...
<body style="margin:0;padding:0;background:#F0F4F8;font-family:Arial,Helvetica,sans-serif;">
  ...
  /**
   * Notify the THERAPIST of a new booking REQUEST awaiting their approval
   * (as opposed to sendBookingNotificationToTherapist, which announces an
   * already-confirmed booking).
   */
  static async sendBookingRequestNotificationToTherapist({ toEmail, toName, clientName, clientEmail, clientPhone, sessionTime, modality, durationMin = 60, clinicName = 'Neure Prodesk' }) {
    const htmlContent = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head>
<body style="margin:0;padding:0;background:#F0F4F8;font-family:Arial,Helvetica,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#F0F4F8;padding:32px 16px;">
    <tr><td align="center">
      <table cellpadding="0" cellspacing="0" style="width:100%;max-width:540px;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.10);">
        ...
        <tr>
          <td style="background:#FFFFFF;padding:28px 28px 24px;">
            <p style="margin:0 0 4px;font-size:22px;font-weight:700;color:#111827;">New Booking Request ⏳</p>
            <p style="margin:0 0 22px;font-size:13px;color:#C89364;font-weight:500;">Awaiting your approval</p>
            <p style="margin:0 0 22px;font-size:15px;color:#4B5563;line-height:1.7;">
              Hi <strong style="color:#111827;">${toName}</strong>,<br>
              <strong style="color:#111827;">${clientName}</strong> has requested a session with you. Please review and approve or decline it in ProDesk.
            </p>
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#F8FAFB;border:1px solid #E5EAF0;border-radius:10px;margin-bottom:20px;">
              <tr>
                <td style="padding:14px 20px;border-bottom:1px solid #E5EAF0;">
              ...
              <tr>
                <td style="padding:14px 20px;border-bottom:1px solid #E5EAF0;">
                  <p style="margin:0 0 4px;font-size:11px;font-weight:700;letter-spacing:0.06em;text-transform:uppercase;color:#9CA3AF;">Requested Date &amp; Time</p>
                  <p style="margin:0;font-size:15px;font-weight:600;color:#111827;">${sessionTime} IST</p>
                </td>
              </tr>
              <tr>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
              <tr>
                <td style="background:#FFF8F0;border-left:3px solid #C89364;border-radius:0 6px 6px 0;padding:12px 16px;">
                  <p style="margin:0;font-size:13px;color:#374151;line-height:1.65;">
                    This slot is held for now, but the client won't receive confirmation until you approve the request.
                  </p>
                </td>
              </tr>
            </table>
    ...
    try {
      const response = await axios.post(
        'https://api.brevo.com/v3/smtp/email',
        {
          sender: { name: BREVO_SENDER_NAME, email: BREVO_SENDER_EMAIL },
          to: [{ email: toEmail, name: toName }],
          subject: `New booking request: ${clientName} for ${sessionTime}`,
          htmlContent
        },
        { headers: { 'api-key': await getBrevoApiKey(), 'Content-Type': 'application/json' } }
      );
      ...
      status = 'FAILED'; statusCode = err.response?.status || 500;
      console.error('sendBookingRequestNotificationToTherapist error:', err.message);
    }

    await NotificationService.logNotification({
      platform: 'EMAIL', status_code: statusCode, status,
      type: 'BOOKING_REQUEST_NOTIFICATION_THERAPIST',
      message: `booking_request_notification_therapist → ${toEmail}`
    });
  }
